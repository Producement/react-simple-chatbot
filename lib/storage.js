  ...
  if (cache && unParsedCache) {
    try {
      ...
      for (let i = 0, len = data.renderedSteps.length; i < len; i += 1) {
        ...
        // if (i === len - 1 && !currentStep.user && !data.renderedSteps[i].user)
      ...
      if (data.currentStep.user) {
        data.currentStep.validator = steps[currentStep.id].validator;
        data.currentStep.parser = steps[currentStep.id].parser;
      }
