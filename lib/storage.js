...
const getData = async (params, callback) => {
  const { cacheName, cache, firstStep, steps, getNextStepFromApi } = params;
  ...
  if (cache && unParsedCache) {
    try {
      ...
      if (!steps[id]) {
        steps[id] = await getNextStepFromApi(id);
      }
      ...
      // waiting user type
      if (waitingForUserInput) {
        if (steps[data.currentStep.id].validator)
          data.currentStep.validator = steps[data.currentStep.id].validator;

        if (steps[data.currentStep.id].parser)
